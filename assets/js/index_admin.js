...
function BorrarUsuario(IdUser, Estado) {
    ...
    swal({
        ...
        $.ajax({
            url: "EliminarUsuario/" + IdUser + "/" + Estado,
            type: "post",
            async: true,
            success: function() {
                swal({
                    title: "EL USUARIO SE CAMBIO CORECTAMENTE!",
                    type: "success",
                    confirmButtonText: "CERRAR",
                }).then(
                    function() { gotopage("Usuarios"); }
...
function BorrarTrabajador(IdUser, Estado) {
    if (Estado == 1) {
        var miMSS = "¿DESEA CAMBIAR EL ESTADO ACTIVO AL TRABAJADOR?";
    ...
    swal({
        ...
        $.ajax({
            url: "EliminarTrabajador/" + IdUser + "/" + Estado,
            type: "post",
            async: true,
            success: function() {
                swal({
                    title: "EL TRABAJADOR SE CAMBIO CORECTAMENTE!",
                    type: "success",
                    confirmButtonText: "CERRAR",
                }).then(
                    function() { gotopage("Trabajadores"); }
    ...
    var calendar = $("div#calendar").fullCalendar({
                            ...
                            swal({
                                ...
                                preConfirm: function(value) {
                                    return new Promise(function(resolve, reject) {
                                        setTimeout(function() {
                                            if (value == '') {
                                                reject('Los Puntos no pueden ser Vacios!')
                                            } else { resolve() }
                                        }, 900)
                                    })
                                },
                                ...
                                $.ajax({
                                    url: "GCalendario/" + Iduser + "/" + value + "/" + Fecha,
                                    type: "post",
                                    async: true,
                                    success: function() {
                                        swal({
                                            title: "LOS PUNTOS SE AGREGARON CORECTAMENTE!",
                                            type: "success",
                                            confirmButtonText: "CERRAR",
        ...
        eventClick: function(event) {
                swal({
                        ...
                        $.ajax({
                            url: "UCalendario/" + Iduser + "/" + event.Id + "/" + value + "/" + Fecha,
                            type: "post",
                            async: true,
                            success: function() {
                                swal({
                                    title: "LOS PUNTOS SE ACTUALIZARON CORECTAMENTE!",
                                    type: "success",
                                    confirmButtonText: "CERRAR",
                                }).then(function() { calendar.fullCalendar("refetchEvents"); })
                            }
...
function EnviarReporte() {
    ...
    $.ajax({
        ...
        success: function() {
            swal({
                title: "EL REPORTE SE ACTUALIZO CON EXITO!",
                type: "success",
                confirmButtonText: "CERRAR",
            }).then(
        ...
        error: function() {
            swal({
                title: "EL REPORTE NO SE PUDO ACTUALIZAR!",
                type: "error",
                confirmButtonText: "CERRAR",
            }).then(
                function() { gotopage("CalSemana"); }
